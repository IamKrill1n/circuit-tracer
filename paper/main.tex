...
\]
so that each column of $A^{\mathrm{in}}$ sums to 1 (incoming mass normalized per node) and each row of $A^{\mathrm{out}}$ sums to 1 (outgoing mass normalized per node). The indirect influence and relevance matrices are the Neumann series
\[
B^{\mathrm{in}} = (I-(A^{\mathrm{in}})^{\top})^{-1}, \quad
B^{\mathrm{out}} = (I-A^{\mathrm{out}})^{-1},
\]
which converge because $\cG$ is a DAG. Let $\ell\in\R^n$ denote the logit-weight vector ($\ell_u=0$ off the target logits, positive on them) and $\tau\in\R^n$ the token-weight vector, with $\tau_u=0$ off embedding nodes and $\tau_u\ge 0$ on input tokens. We set $\tau$ to normalized SHAP values \cite{shap}, providing a sparse, human-aligned per-token importance signal. The node-level scores are
\[
\mathrm{Inf}_u = (\ell^\top B^{\mathrm{in}})_u, \qquad
\mathrm{Rel}_u = (\tau^\top B^{\mathrm{out}})_u,
\]
and the edge-level scores are
\[
\mathrm{Inf}_{uv} = \mathrm{Inf}_v\,A^{\mathrm{in}}_{vu}, \qquad
\mathrm{Rel}_{uv} = \mathrm{Rel}_u\,A^{\mathrm{out}}_{uv}.
\]

\paragraph{Combined score.}
Influence and relevance live on different scales: both distributions are heavily zero-inflated and a small number of nodes dominate the raw mass, so directly comparing or summing the two vectors is unreliable. We therefore first normalize each score to a common $[0,1]$ scale and then combine them. Let $s\in\R^N$ be a score vector (either $\mathrm{Inf}$ or $\mathrm{Rel}$, evaluated over nodes or over the flattened edge set). We use the rank-percentile transform
\[
\mathrm{rank}(s)_u \;=\; \frac{|\{v : s_v < s_u\}|}{N - 1} \;\in\; [0,1],
\]
which is robust to zero-inflation and to outliers. Writing $\widehat{\mathrm{Inf}}=\mathrm{rank}(\mathrm{Inf})$ and $\widehat{\mathrm{Rel}}=\mathrm{rank}(\mathrm{Rel})$, the combined score with mixing parameter $\alpha\in[0,1]$ is the weighted geometric mean
\[
C_u \;=\; \bigl(\widehat{\mathrm{Inf}}_u + \varepsilon\bigr)^{\alpha}\,\bigl(\widehat{\mathrm{Rel}}_u + \varepsilon\bigr)^{1-\alpha}.
\]
The geometric form penalizes nodes that are weak on either axis, encoding the desideratum that a retained node should be \emph{both} reachable from the input and influential on the output. Arithmetic and harmonic means are also supported as alternative combiners.

We motivate the use of a combined score over the alternative of separately thresholding $\mathrm{Inf}$ and $\mathrm{Rel}$ and intersecting the resulting masks. The intersection rule drops every node that fails either independent threshold even if it scores moderately on both, and this loss is non-trivial in practice: empirically, the rank-percentile correlation between $\mathrm{Inf}$ and $\mathrm{Rel}$ on feature nodes is mildly positive (Pearson $r \approx 0.2$--$0.4$ on multihop prompts), so the two signals carry complementary information yet are not orthogonal. A single combined score with a single threshold yields a strictly more principled ranking and avoids the asymmetry of two independent cutoffs.
...
We apply this rule once at the node level with threshold $\eta_V$ to obtain a node mask, then recompute the edge-level combined score $C_{uv}$ on the node-pruned graph and apply the rule again at the edge level with threshold $\eta_E$. A final cleanup step removes dangling nodes (interior nodes with no surviving outgoing or incoming edges) until the masks reach a fixed point.

\begin{algorithm}[H]
\caption{Prune attribution graph (combined score)}
\label{alg:prune}
\begin{algorithmic}[1]
\Require Graph $\cG$, \texttt{token\_weights}, \texttt{logit\_weights}, thresholds $(\eta_V,\eta_E)$, mixing $\alpha$
\Ensure $(\texttt{node\_mask},\texttt{edge\_mask})$
\State Compute $\mathrm{Inf}, \mathrm{Rel}$ on nodes via Neumann series
...
\State $\texttt{node\_mask} \gets M_{\eta_V}(C^V)$;\quad force-keep tokens and target logits
\State Recompute $\mathrm{Inf}_{uv}, \mathrm{Rel}_{uv}$ on the node-pruned graph
...
\State Iteratively remove dangling nodes and edges
