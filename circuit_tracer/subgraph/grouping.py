...
def _default_prunegraph_candidates(pg: PruneGraph) -> List[str]:
    return [
        nid
        for nid in pg.kept_ids
        if pg.attr.get(nid, {}).get("feature_type") == "cross layer transcoder"
    ]


def prunegraph_to_nx(
    pg: PruneGraph,
    candidate_node_ids: Optional[List[str]] = None,
    use_abs_weight: bool = True,
) -> nx.DiGraph:
    """
    Build a DiGraph from PruneGraph using pruned_adj and kept_ids.
    Nodes are raw node_id strings.
    """
    if candidate_node_ids is None:
        candidate_node_ids = _default_prunegraph_candidates(pg)

    id_to_idx = {nid: i for i, nid in enumerate(pg.kept_ids)}
    nodes = [nid for nid in candidate_node_ids if nid in id_to_idx]
    ...
    idx = [id_to_idx[n] for n in nodes]
    sub_adj = pg.pruned_adj[idx][:, idx].detach().cpu().numpy()
...
def group_prune_graph(
    prune_graph: PruneGraph,
    distance_graph: np.ndarray,
    candidate_node_ids: Optional[List[str]] = None,
    conditions: Optional[Callable[[Any, Any, nx.DiGraph], bool]] = None,
    num_groups: Optional[int] = None,
) -> Tuple[List[Tuple[Any, ...]], nx.DiGraph, List[List[str]]]:
    """
    Top-level grouping entrypoint that accepts:
      - prune_graph: PruneGraph result from prune_graph_pipeline
      - distance_graph: dense NxN distance matrix produced separately (distance.py)
      - candidate_node_ids: subset of prune_graph.kept_ids corresponding to rows/cols of distance_graph
    Returns:
      - groups: list of tuples (clusters of original node ids)
      - merged_graph: networkx DiGraph after merging
      - supernodes: Neuronpedia-style list of [label, node_id, ...]
    """
    # If candidate_node_ids not provided assume distance_graph rows/cols map to CLT candidate nodes
    if candidate_node_ids is None:
        candidate_node_ids = _default_prunegraph_candidates(prune_graph)

    # Build graph restricted to candidate nodes
    G = prunegraph_to_nx(prune_graph, candidate_node_ids=candidate_node_ids)

    # Sanity checks for distance_graph shape
    if distance_graph is None:
        raise ValueError("distance_graph must be provided (use distance.build_* on PruneGraph).")
    if distance_graph.shape[0] != distance_graph.shape[1]:
        raise ValueError("distance_graph must be square.")
    if distance_graph.shape[0] != len(list(G.nodes())):
        raise ValueError(
            f"distance_graph size ({distance_graph.shape[0]}) doesn't match number of candidate nodes ({len(list(G.nodes()))})."
        )

    # Call existing greedy_grouping on the nx graph using provided distance_graph
    groups, merged_graph = greedy_grouping(
        graph=G,
    ...
    # Build supernodes (Label + members)
    supernodes: List[List[str]] = []
    for i, g in enumerate(groups):
        members = list(g) if isinstance(g, tuple) else [g]
        clerps = [
            prune_graph.attr.get(m, {}).get("clerp", "").strip()
            for m in members
            if isinstance(m, str)
        ]
        clerps = [c for c in clerps if c]
        label = (sorted(clerps, key=len)[0][:80] if clerps else f"cluster_{i+1}")
        supernodes.append([label, *members])

    return groups, merged_graph, supernodes
