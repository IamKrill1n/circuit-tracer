...
from circuit_tracer.graph import Graph, prune_graph, topk_prune_graph
...
def create_graph_files_topk(
    ...
    graph.to(device)
    node_mask, edge_mask, cumulative_scores = (
        el.cpu() for el in topk_prune_graph(graph, top_k)
    )
    graph.to("cpu")
    ...
    nodes = create_nodes(graph, node_mask, tokenizer, cumulative_scores)
    used_nodes, used_edges = create_used_nodes_and_edges(graph, nodes, edge_mask)
    model = build_model(graph, used_nodes, used_edges, slug, scan, node_threshold=0.0, tokenizer=tokenizer)
